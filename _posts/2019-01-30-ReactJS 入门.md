...
layout: post
title: ReactJS 入门
description: React 以组件化的开发方式，专注于 MVC 架构中的 View，即视图， 这使得React很容易和开发者已有的开发栈进行融合。React 推荐将 UI 上每一个功能相对独立的模块定义成组件，然后将小的组件通过组合或者嵌套的方式构成大的组件，最终完成整体 UI 的构建。
tags: [ReactJS]
---
...
一般我们启动一个 React 项目会使用 React 脚手架工具 [create-react-app](https://github.com/facebook/create-react-app),它会帮助你创建一个基于 webpack、Babel 和 ESLint 的单页面项目。
...
ReactDOM.render(
  <WebSite name="Leo" site="https://fshd.com" />,
  document.getElementById('root')
);
```